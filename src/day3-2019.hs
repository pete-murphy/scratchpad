...
solution str =
  str
    & lines
    & map (toTokens >>> map parseToken >>> toPoints')
    & (\(xs : ys : _) -> S.fromList xs `S.intersection` S.fromList ys)
    & S.delete (0, 0)
    & minimumBy (comparing manhattanDistance)
    & manhattanDistance
...
parseToken (d : n) = d'
  where
    n' = read n :: Int
    d' = \(x, y) -> case d of
      'L' -> [x - n' .. x] `zip` repeat y
      'R' -> reverse [x .. x + n'] `zip` repeat y
      'U' -> repeat x `zip` [y - n' .. y]
      'D' -> repeat x `zip` reverse [y .. y + n']
...
--          => (a -> f b) -> t a -> f (t b)
-- traverse :: (State Point, Traversable t)
--          => (a -> State Point b) -> t a -> State Point (t b)
-- traverse :: (State Point, [])
--          => (a -> State Point b) -> [a] -> State Point [b]
-- traverse :: (Token -> State Point b) -> [Token] -> State Point [b]
-- traverse :: (Token -> State Point [Point]) -> [Token] -> State Point [[Point]]
